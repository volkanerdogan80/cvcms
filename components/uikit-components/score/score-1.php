<?php if (is_post()): ?>
    <?php $is_rating = is_rating(); $score_list = cve_post_rating_score(); ?>
                ...
                <div>
                    <div class="rating">
                        <?php foreach ($score_list as $vote => $score): ?>
                            <input <?= $is_rating == $vote ? 'checked' : ''; ?> type="radio" data-url="<?= cve_route('content_vote') ?>" name="rating" class="uikit-voted" data-content="<?= cve_post_id() ?>" data-vote="<?= $vote ?>" id="rating-<?= $vote ?>">
                    ...
                    <?php foreach ($score_list as $vote => $score): ?>
                        <div uk-grid style="margin-top: 0">
                            <div class="uk-width-2-5@m uk-text-center">
                                <?= $vote ?> <?= cve_admin_lang('Inputs', 'star') ?> (<span class="uikit-<?= $vote ?>-text"><?= $score ?></span>)
                            </div>
                            <div class="uk-width-3-5@m">
                                <progress style="margin-bottom: 0" class="uk-progress uikit-<?= $vote ?>-value" value="<?= $score ?>" max="100"></progress>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
