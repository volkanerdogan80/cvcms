<div class="modal-header">
    <h5 class="modal-title"><?= cve_admin_lang_path('Comments', 'reply_comment') ?></h5>
    <button type="button" class="close" data-dismiss="modal">&times;</button>
</div>
<div class="modal-body comment-modal">
    <input type="hidden" value="<?= $comment->id ?>" id="comment_id">
    <div class="form-group">
        <label class="col-form-label"><?= cve_admin_lang_path('Comments', 'your_comment') ?></label>
...
<div class="modal-footer">
    <button type="button" class="btn btn-outline-primary btn-block comment-reply-send" data-url="<?= base_url(route_to('admin_comment_reply')) ?>" ><?= cve_admin_lang_path('Buttons', 'send') ?></button>
</div>
