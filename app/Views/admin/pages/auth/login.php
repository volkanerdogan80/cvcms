        ...
        <div class="d-flex flex-wrap align-items-stretch">
            <div class="col-lg-4 col-md-6 col-12 order-lg-1 min-vh-100 order-2 bg-white">
                <div class="p-4 m-3">
                    ...
                    <h4 class="text-dark font-weight-normal"><?= cve_admin_lang_path('Auth', 'welcome') ?></h4>
                    <p class="text-muted"><?= cve_admin_lang_path('Auth', 'start') ?></p>
                    ...
                    <?php if(!config('system')->login): ?>
                        <div class="alert alert-warning alert-has-icon">
                            ...
                            <div class="alert-body">
                                ...
                                <?= cve_admin_lang_path('errors', 'login_system_passive') ?>
                            </div>
                        </div>
                    <?php endif; ?>
                    ...
                    <form method="POST" action="<?= base_url(route_to('admin_login')); ?>" class="needs-validation" novalidate="">
                        ...
                        <div class="form-group">
                            <div class="d-block">
                                <label for="password" class="control-label"><?= cve_admin_lang_path('Inputs', 'password') ?></label>
                            </div>
                        ...
                        <div class="mt-5 text-center">
                            <?= cve_admin_lang_path('Auth', 'no_account') ?>
                            <a href="<?= base_url(route_to('admin_register')); ?>"><?= cve_admin_lang_path('Auth', 'create_account') ?></a>
                        </div>
                    </form>
                    ...
                    <div class="text-light p-5 pb-2">
                        <div class="mb-5 pb-3">
                            <?php if ($time->getHour() > 5 && $time->getHour() <= 11){ ?>
                                <h1 class="mb-2 display-4 font-weight-bold"><?= cve_admin_lang_path('Auth', 'good_morning') ?></h1>
                            <?php }elseif($time->getHour() > 11 && $time->getHour() <= 16){ ?>
                                <h1 class="mb-2 display-4 font-weight-bold"><?= cve_admin_lang_path('Auth', 'good_afternoon') ?></h1>
                            <?php }elseif($time->getHour() > 16 && $time->getHour() <= 22){ ?>
                                <h1 class="mb-2 display-4 font-weight-bold"><?= cve_admin_lang_path('Auth', 'good_evening') ?></h1>
                            <?php }else{ ?>
                                <h1 class="mb-2 display-4 font-weight-bold"><?= cve_admin_lang_path('Auth', 'goodnight') ?></h1>
                            <?php } ?>
